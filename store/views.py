...
def store(request,category_slug=None):
    ...
    context = {'products': paged_products, 'product_count': product_count,'links': all_categories}
...
def product_detail(request, category_slug, product_slug):
    ...
    return render(request, 'store/product_detail.html', context)
# def product_detail(request,category_slug,product_slug):
#     print('slug from url',product_slug)
#     single_product = get_object_or_404(Product,category__slug=category_slug,slug=product_slug)
#     in_cart=CartItem.objects.filter(cart__cart_id=_cart_id(request), product=single_product).exists()
#     print(f'debug:{in_cart}')
...
#                 return redirect(url)
#             else:
#                 messages.error(request,'Review failed')
#                 return redirect(url)
#     return redirect(url)
...
def submit_review(request, product_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            form = ReviewForm(request.POST, instance=reviews)
            form.save()
            messages.success(request, 'Review updated successfully')  # Update case
            return redirect(url)
        except ReviewRating.DoesNotExist:
            form = ReviewForm(request.POST)
            if form.is_valid():
                ...
                # 🔹 YE LINE ADD KARO (Sabse important)
                data.status = True
